...
<head>
    ...
    <title>SEBIDUK Linggau | {{ $title }}</title>
    {{-- CSS Utama --}}
...
<body
    ...
    <div class="wrapper">
        ...
        <div id="my-element" class="preloader flex-column justify-content-center align-items-center">
            <img class="animation__shake" src="{{ asset('template/landingPage/assets/img/') }}/sebiduklinggau.png"
                alt="AdminLTELogo" height="150px">
        </div>
    ...
    {{-- Seacrh Kategori KBLI --}}
    <script>
        $(document).ready(function() {
            $('#kbliSelect').on('select2:select', function(e) {
                var kategori = e.params.data.element.dataset.kategori || '';
    ...
    <script>
        $(document).ready(function() {
            // Target checkbox dengan ID yg benar
            $("#internet1").change(function() {
                if ($(this).is(":checked")) {
                    $("#mediaPenjualanSection").slideDown();
                } else {
                    $("#mediaPenjualanSection").slideUp();
                    $("#mediaPenjualanSection input[type=checkbox]").prop("checked", false);
                }
            });

            // Tambahkan ini jika ingin auto-show saat halaman reload dan sudah dicentang
            if ($("#internet1").is(":checked")) {
                $("#mediaPenjualanSection").show();
            }
        });
    </script>
    <script>
        function showBlock(selection) {
            // Sembunyikan semua blok dulu
            $(".card-block").hide();

            if (selection === '1-2') {
                // Tampilkan Blok I dan II
                $("#blok1").show();
                $("#blok2").show();
            } else if (selection === 3) {
                // Tampilkan Blok III
        ...
        // Saat pertama kali load, tampilkan Blok I dan II
        $(document).ready(function() {
            showBlock('1-2');
        });
    </script>
